...
function _CoreUI_Js_getJsBundle() {
  ...
  '  // Bullet characters Word emits after normalization strips class/style',
  '  var BULLET_CHARS = [\'\\u00b7\', \'\\u2022\', \'\\u25e6\', \'\\u25aa\', \'\\u25cf\',',
  '                      \'\\u25cb\', \'\\u2013\', \'\\u2014\', \'-\', \'*\'];',
  '  var NUMERIC_RE   = /^(\\d+|[a-zA-Z]|[ivxlcdmIVXLCDM]+)[.)][\\u00a0\\s]/;',
  '',
  ...
  '    for (var b = 0; b < BULLET_CHARS.length; b++) {',
  '      if (first === BULLET_CHARS[b]) return \'ul\';',
  '    }',
  '    if (NUMERIC_RE.test(text)) return \'ol\';',
  '    return null;',
  '  }',
  '',
  '  function stripMarker(p) {',
  '    var node = p.firstChild;',
  '    // Remove empty leading text nodes (pure whitespace/nbsp)',
  ...
  '    var val = node.nodeValue.replace(/^[\\u00a0\\s]*/, \'\');',
  '    // Try stripping numeric marker first',
  '    var stripped = val.replace(/^(\\d+|[a-zA-Z]|[ivxlcdmIVXLCDM]+)[.)][ \\u00a0]+/, \'\');',
  '    if (stripped === val) {',
  '      // Try stripping bullet character',
  '      stripped = val.replace(/^.[\\u00a0\\s]+/, \'\');',
  '    }',
  '    if (!stripped) {',
  '      p.removeChild(node);',
  '    } else {',
  ...
  '        }',
  '      }',
  '      // Build the list element and convert each <p> to <li>',
