...
def coerce_excel_date_header(value):
    """
    Return a pandas Timestamp for a date-like weekly-header value, otherwise None.

    Supports:
    - Python/Pandas datetime values
    - Excel numeric date serials, such as 46144
    - Text Excel date serials, such as '46,144.00'
    - Common date strings, such as 05/02/2026 or 2026-05-02
    """
    if pd.isna(value):
        return None
    ...
    if isinstance(value, (int, float)) and not isinstance(value, bool):
        serial = float(value)
        if 1 <= serial <= 60000:
            return pd.to_datetime(serial, unit="D", origin="1899-12-30").normalize()

    text = str(value).strip()
    if not text:
        return None

    # Handles source headers such as "46,144.00".
...
def normalized_week_header(value):
    """
    Return a canonical MM/DD/YYYY weekly header string when value is date-like.
    Return the original value unchanged when it is not a date header.
    """
    parsed = coerce_excel_date_header(value)
    return parsed.strftime("%m/%d/%Y") if parsed is not None else value

def is_date_header(value) -> bool:
    return coerce_excel_date_header(value) is not None
...
def fiscal_quarter_from_history_label(value) -> str:
    """
    Normalize one source Fiscal Qtr value to FY##-Q# without hardcoding
    fiscal periods.
    ...
    # Do not silently assign a row that references multiple quarters.
    if len(re.findall(r"\bQ\s*[1-4]\b", text)) != 1:
        return ""

    patterns = [
        (
            r"\bFY\s*(\d{2,4})\s*[-_/\s]*Q\s*([1-4])\b",
            "fy_first",
        ),
        (
            r"\bQ\s*([1-4])\s*[-_/\s]*FY\s*(\d{2,4})\b",
            "q_first",
        ),
        (
            r"\b(\d{4})\s*[-_/\s]*Q\s*([1-4])\b",
            "fy_first",
        ),
        (
            r"\bQ\s*([1-4])\s*[-_/\s]*(\d{4})\b",
            "q_first",
        ),
    ]
    ...
    for pattern, order in patterns:
        ...
        if order == "fy_first":
            fiscal_year, quarter = match.group(1), match.group(2)
        else:
            quarter, fiscal_year = match.group(1), match.group(2)

        return f"FY{fiscal_year[-2:]}-Q{quarter}"
...
def load_forecast_wide(xl, log):
    ...
    hdr = [normalized_week_header(value) for value in raw.iloc[header_row]]
...
def coerce_week_header(value):
    """
    Return a normalized Timestamp for a weekly date-header value, or None.

    Handles:
    - Excel/Pandas datetime values
    - Numeric Excel date serials, such as 46144
    - Text Excel serials, such as "46,144.00"
    - MM/DD/YYYY and ISO-style date strings
    """
    if value is None or pd.isna(value):
        return None
    ...
    # Supports text Excel serials such as "46,144.00".
...
def load_util_sheet(xl, sheet, log):
    ...
    hdr = next(
        (
            ...
            if str(raw.iloc[i, 0]).strip() == "Employee ID"
        ),
        None,
    )
    ...
    if hdr is None:
        raise ValueError(
            f"{sheet}: could not locate 'Employee ID' header row."
        )

    qk = next(
        (
            fiscal_quarter_from_util_title(raw.iloc[i, 0])
            for i in range(hdr)
            if fiscal_quarter_from_util_title(raw.iloc[i, 0])
        ),
        None,
    )
    ...
    if qk is None:
        ...
        if not wk:
            raise ValueError(
                f"{sheet}: no title range and no weekly date headers "
                "to derive quarter."
            )
    ...
    # Retain all columns, including the weekly date columns.
    # Converting column names to strings preserves the dates as
    # ISO-style strings such as 2026-05-02 00:00:00.
...
def build_current_actuals(xl, log):
    ...
    if coerce_excel_date_header(column) is not None
    ]

    if not weekly:
        # load_util_sheet converts datetime headers to strings such as
        # 2026-05-02 00:00:00, so detect both ISO and MM/DD/YYYY forms.
        weekly = [
            column
            for column in df.columns
            if re.fullmatch(
    ...
    if not weekly:
        # First week of a new current quarter: the weekly date headers may not
        # exist yet (no week has actualized). This is a VALID state, not an error —
        # emit an empty current-actuals table and continue. Actuals accrue week by
        # week and will populate on subsequent exports.
        log.append(
            "UTIL_Current: no dated weekly actual columns yet "
            "(expected at the start of a quarter) — 0 current-actual rows."
        )
        return pd.DataFrame(
            columns=[
                "employee_id", "resource_name", "week_start",
                "week_key", "actual_icp_hours", "source_row",
            ]
    )

    log.append(
        f"UTIL_Current: detected {len(weekly)} weekly actual column(s): "
        + ", ".join(str(column) for column in weekly)
    )
    ...
    if act["week_start"].isna().any():
        ...
        raise ValueError(
            "UTIL_Current: unable to convert one or more weekly headers: "
            + ", ".join(repr(value) for value in bad_headers)
        )
...
def build_history(xl, log):
    ...
    missing = [column for column in req if column not in src.columns]
    if missing:
        raise ValueError(
            "Actuals_Historical missing: " + ", ".join(missing)
        )

    src = src[~src["Worker"].map(is_junk_worker)].copy()
    src["Worked Hours"] = safe_numeric(src["Worked Hours"])

    # Drop only blank trailing/export rows. A row with actual worked hours
    # is never discarded merely because its Fiscal Qtr is blank.
    blank_export_rows = (
        src["Fiscal Qtr"].isna()
        & src["Project"].isna()
        & src["Worked Hours"].eq(0)
    )

    blank_export_count = int(blank_export_rows.sum())

    if blank_export_count:
        log.append(
            "Actuals_Historical: dropped "
            f"{blank_export_count} blank trailing/export row(s)"
        )
        src = src.loc[~blank_export_rows].copy()

    src["employee_id"] = src["Worker ID"].map(normalize_employee_id)

    # Preserve the source-report value for the Stage 4 Workday Region drill-down.
    src["workday_region_as_of_date_worked"] = (
        src["Region as of Date Worked"]
        .fillna("")
        .astype(str)
        .str.strip()
        .replace(
        ...
        # Specialty Practice — forward-demand dimension (HCM, FIN, Integrations, etc.).
    src["specialty_practice"] = (
        src["Specialty Practice"]
        .fillna("")
        .astype(str)
        .str.strip()
        .replace(
    ...
    # Sub-Specialty Practice — historical breakdown context only.
    src["sub_specialty_practice"] = (
        src["Sub-Specialty Practice"]
        .fillna("")
        .astype(str)
        .str.strip()
        .replace(
    ...
    # Retain the existing worker-class rules.
    src["worker_class"] = [
        classify_history_worker(worker_name, region)
        for worker_name, region in zip(
            src["Worker"],
            src["Region as of Date Worked"],
        )
    ]

    src["fiscal_quarter"] = src["Fiscal Qtr"].map(
        fiscal_quarter_from_history_label
    )

    bad_mask = src["fiscal_quarter"] == ""
    bad = int(bad_mask.sum())

    if bad:
        invalid_rows = src.loc[
            bad_mask,
            [
                "Worker ID",
                "Worker",
        ...
        raise ValueError(
            "Actuals_Historical: "
            f"{bad} row(s) have a missing or invalid Fiscal Qtr after "
            "blank export-row filtering. Sample rows:\n"
            + invalid_rows.to_string(index=False)
        )

    detected_quarters = sorted(src["fiscal_quarter"].unique())

    log.append(
        "Actuals_Historical fiscal quarters detected: "
        + ", ".join(detected_quarters)
    )
    ...
    hist = hist[history_column_order]
    ...
    region_reconciliation = pd.concat(
        [
            source_region_hours.rename("source_hours"),
            output_region_hours.rename("output_hours"),
        ],
        axis=1,
    ).fillna(0)

    max_region_delta = float(
        (
            region_reconciliation["source_hours"]
            - region_reconciliation["output_hours"]
        )
        .abs()
        .max()
    )
    hist["worked_hours"] = hist["worked_hours"].round(2)
    total = round(float(hist["worked_hours"].sum()), 2)
    return hist, total, max_region_delta
...
def validate(
    ...
    # Current actuals — these checks only apply once actuals exist. Early in a
    # quarter the actuals table is legitimately empty (no week has closed yet);
    # treat that as a pass, not a failure.
    if len(actuals) > 0:
        ...
        chk("Actuals_Current: empty at start of quarter (checks skipped)", True)
    ...
    chk("Utilization: exactly 3 fiscal quarters", util["fiscal_quarter"].nunique() == 3)
        ...
        # History
    chk(
        "History: no blank employee_id",
        (hist["employee_id"] == "").sum() == 0,
    )

    chk(
        "History: worked hours reconcile to source",
        abs(hist["worked_hours"].sum() - hist_total) < 0.01,
    )

    chk(
        "History: at least one fiscal quarter detected",
        hist["fiscal_quarter"].nunique() >= 1,
    )

    chk(
        "History: no blank Workday Region",
        hist["workday_region_as_of_date_worked"]
        .fillna("")
        .astype(str)
        .str.strip()
        .ne("")
    ...
    chk(
        "History: Workday Region hours reconcile to source",
        max_region_delta < 0.01,
    )

    chk("History: worker classes valid (subset of SLG / Non-SLG / Contractor)",
        set(hist["worker_class"].unique()).issubset({"SLG", "Non-SLG", "Contractor"})
        and hist["worker_class"].nunique() >= 1)
    return results
...
def build_manifest(staffed, unstaffed, retained_weeks, actuals, util, hist):
    ...
    return pd.DataFrame([
        ...
        {"sheet": OUT_HISTORY, "rows": len(hist), "primary_measure": "worked_hours",
         "primary_total": round(float(hist["worked_hours"].sum()), 2),
...
def main():
    ...
    try:
        ...
        hist, hist_total, max_region_delta = build_history(xl, log)
