...
function _CoreUI_Js_getJsBundle() {
  ...
  '',
  '// State',
  'var mgmPglData   = null;  // { windowDays, today, rows, exceptions }',
  'var mgmPglFilter = { type: \'all\', search: \'\' };',
  '',
  '/**',
  ' * Entry-point: loads MGM/PGL data from the server and renders the tab.',
  ' * Called on tab switch (always reloads to pick up viewMode changes).',
  ' */',
  ...
  '  google.script.run',
  '    .withSuccessHandler(function(data) {',
  '      mgmPglData = data || { windowDays: 30, today: \'\', rows: [], exceptions: [] };',
  ...
  '      if (loadInd) loadInd.classList.add(\'hidden\');',
  '      if (asOf) asOf.textContent = \'Window: next \' + (mgmPglData.windowDays || 30) + \' days from \' + (mgmPglData.today || \'\') + \' \u00B7 loaded \' + _fmtNow_();',
  '    })',
  '    .withFailureHandler(function(err) {',
  '      if (loadInd) loadInd.classList.add(\'hidden\');',
  ...
  '    })',
  '    .getUpcomingSurveysData(getViewModeOpts_());',
  '}',
  '',
  '/**',
  ...
  '    var surveyBadge = r.surveyType === \'MGM\'',
  '      ? \'<span class="health-badge" style="background:var(--color-blue-soft,#e0f2fe);color:#0369a1;border:1px solid #bae6fd;">MGM</span>\'',
  '      : \'<span class="health-badge" style="background:var(--color-purple-soft,#f3e8ff);color:#7c3aed;border:1px solid #ddd6fe;">PGL</span>\';',
  '',
  '    var statusText = r.status ? \'(\' + escapeHtml(r.status) + \')\' : \'\';',
  '',
  ...
  '      \'<td>\' + surveyBadge + \'</td>\' +',
  '      \'<td style="font-size:12px;color:var(--color-text-muted);">\' + escapeHtml(statusText) + \'</td>\' +',
  '      \'<td>\' + escapeHtml(r.accountName    || \'\') + \'</td>\' +',
  '      \'<td>\' + escapeHtml(r.deploymentName || \'\') + \'</td>\' +',
  '      \'<td style="font-size:12px;color:var(--color-text-muted);">\' + escapeHtml(r.productLabel || \'\') + \'</td>\' +',
  '      \'<td style="font-size:12px;color:var(--color-text-subtle,#94a3b8);">(Contacts to be wired in v2)</td>\' +',
  '    \'</tr>\';',
  '  }).join(\'\');',
  '}',
  '',
