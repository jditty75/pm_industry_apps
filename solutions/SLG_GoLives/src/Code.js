...
function doGet() {
  return HtmlService
    ...
    .evaluate()
    .setTitle('State & Local Government Go Lives')
    .setXFrameOptionsMode(HtmlService.XFrameOptionsMode.ALLOWALL);
}

/**
 * Folder where all SLG Slides exports will be stored.
 * SlideExports in the SLG Shared drive.
 */
const EXPORT_FOLDER_ID = '1SWH7ARQuhSoczQeeYK-K1cM8RBxGPYqz';
...
function onOpen() {
  GoLives.buildMenu('SLG Go Lives');
}
...
function buildGoLivesSheet() {
  GoLives.buildGoLivesSheetForSLG();
}
 ...
 * SLG includes deployment partner information.
 */
...
function createSlidesFromFilteredGoLives(payload) {
  ...
  let deckTitlePrefix;
  if (view === 'qmp') {
    deckTitlePrefix = 'SLG Quarterly Meeting Prep Go Lives Export';
  } else if (view === 'upcoming') {
    deckTitlePrefix = 'SLG Upcoming Go Lives Export';
  } else {
    deckTitlePrefix = 'SLG Past Go Lives Export';
  }
...
function testSlidesAuth() {
  const pres = SlidesApp.create('Auth Test - SLG Go Lives');
