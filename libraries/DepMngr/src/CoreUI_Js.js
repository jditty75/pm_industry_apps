...
function _CoreUI_Js_getJsBundle() {
  ...
  '  if (!data || data.error) {',
  '    var tab = document.getElementById(\'overview-tab\');',
  '    if (tab) tab.innerHTML = \'<div style="padding:2rem; color:var(--color-status-red-fg);">Overview data unavailable.</div>\';',
  '    return;',
  '  }',
  ...
  '',
  '  // ── KPI strip ──────────────────────────────────────────────────',
  '  var kpiStrip = document.getElementById(\'overview-kpi-strip\');',
  '  if (kpiStrip) {',
  '    kpiStrip.innerHTML =',
  '      \'<div class="stat-card"><h3>Total Active</h3><div class="value">\' + (dep.total || 0) + \'</div></div>\' +',
  '      \'<div class="stat-card red"><h3>Red</h3><div class="value">\' + (dep.red || 0) + \'</div></div>\' +',
  '      \'<div class="stat-card yellow"><h3>Yellow</h3><div class="value">\' + (dep.yellow || 0) + \'</div></div>\' +',
  '      \'<div class="stat-card green"><h3>Green</h3><div class="value">\' + (dep.green || 0) + \'</div></div>\' +',
  '      \'<div class="stat-card"><h3>Executive Watch</h3><div class="value">\' + (ew.count || 0) + \'</div></div>\';',
  '  }',
  '',
  '  // ── Go-Lives strip ─────────────────────────────────────────────',
  '  var glStrip = document.getElementById(\'overview-golives-strip\');',
  '  if (glStrip) {',
  '    glStrip.innerHTML =',
  '      \'<div class="stat-card"><h3>Go-Lives Last 30d</h3><div class="value">\' + (gl.recentCount || 0) + \'</div></div>\' +',
  '      \'<div class="stat-card"><h3>Go-Lives Next 30d</h3><div class="value">\' + (gl.upcomingCount || 0) + \'</div></div>\';',
  '  }',
  '',
  '  // ── Top Red list ────────────────────────────────────────────────',
  '  var topRedEl = document.getElementById(\'overview-top-red\');',
  '  if (topRedEl) {',
  '    var topRed = dep.topRed || [];',
  '    if (!topRed.length) {',
  '      topRedEl.innerHTML = \'<p style="color:var(--color-text-muted); font-size:0.875rem;">No red deployments.</p>\';',
  '    } else {',
  '      topRedEl.innerHTML = topRed.map(function(r) {',
  '        return \'<div style="padding:var(--space-2) 0; border-bottom:1px solid var(--color-border);">\' +',
  '               \'  <div style="font-weight:600; font-size:0.875rem;">\' + escapeHtml(r.accountName || \'—\') + \'</div>\' +',
  '               \'  <div style="font-size:0.8rem; color:var(--color-text-muted);">\' + escapeHtml(r.deploymentName || \'—\') + \'</div>\' +',
  '               \'  <div style="font-size:0.8rem; color:var(--color-text-subtle);">MTP: \' + (r.mtpDate ? formatDate(r.mtpDate) : \'—\') +',
  '               \'  · \' + escapeHtml(r.partner || \'—\') + \'</div>\' +',
  '               \'</div>\';',
  '      }).join(\'\');',
  '    }',
  '  }',
  ...
  '  if (upcomingEl) {',
  '    var upcoming = gl.upcomingList || [];',
  '    if (!upcoming.length) {',
  '      upcomingEl.innerHTML = \'<p style="color:var(--color-text-muted); font-size:0.875rem;">No upcoming go-lives in the next 30 days.</p>\';',
  '    } else {',
  '      upcomingEl.innerHTML = upcoming.map(function(r) {',
  '        return \'<div style="padding:var(--space-2) 0; border-bottom:1px solid var(--color-border);">\' +',
  '               \'  <div style="font-weight:600; font-size:0.875rem;">\' + escapeHtml(r.accountName || \'—\') + \'</div>\' +',
  '               \'  <div style="font-size:0.8rem; color:var(--color-text-muted);">\' + escapeHtml(r.deploymentName || \'—\') + \'</div>\' +',
  '               \'  <div style="font-size:0.8rem; color:var(--color-text-subtle);">MTP: \' + (r.mtpDate ? formatDate(r.mtpDate) : \'—\') + \'</div>\' +',
  '               \'</div>\';',
  '      }).join(\'\');',
  '    }',
  '  }',
  '',
  '  // ── Stage strip ─────────────────────────────────────────────────',
  '  var stageEl = document.getElementById(\'overview-stage-strip\');',
  '  if (stageEl) {',
  '    var byStage = dep.byStage || [];',
  '    if (!byStage.length) {',
  '      stageEl.innerHTML = \'<span style="color:var(--color-text-muted); font-size:0.875rem;">No stage data.</span>\';',
  '    } else {',
  '      stageEl.innerHTML = byStage.map(function(s) {',
  '        return \'<div style="background:var(--color-surface); border:1px solid var(--color-border); border-radius:var(--radius-md);\' +',
  '               \'padding:var(--space-2) var(--space-3); font-size:0.8rem; white-space:nowrap;">\' +',
  '               \'<strong>\' + escapeHtml(s.stage) + \'</strong> <span style="color:var(--color-text-muted);">\' + s.count + \'</span>\' +',
  '               \'</div>\';',
  '      }).join(\'\');',
  '    }',
  '  }',
