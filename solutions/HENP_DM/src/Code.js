/**
 * HENP Deployment Health – Top-level code wired to CoreLib.
 *
 * Responsibilities:
 * - UI menu and "Open Web App" entry.
 * - HENP-specific TABLES + BAR_CONFIG configuration.
 * - Thin wrappers to CoreLib for:
 *   - updateHealthAnalytics_ (snapshot-based)
 *   - previewHtml (inline HTML preview)
 *   - exportHtmlToDrive (inline + Outlook HTML export)
 * - Optional debug helpers for table detection.
 *
 * NOTE:
 * - Requires APP_CONFIG from Config_HENP.gs
 * - Requires CoreLib library added to project with identifier "CoreLib".
 */
...
// Set this to your deployed Web App URL for HENP
var WEB_APP_URL =
  'https://script.google.com/a/macros/workday.com/s/AKfycbz8eIK0zeEGStLFbs7m_juC_0kf_IDswxLP1SSPZizk_SWP3S8fnTGPhv9M-ahhDBXqAQ/exec';

/**
 * TABLES:
 *  - sheetName : Sheet tab the table lives on.
 *  - title     : Legacy text label for dynamic search/debug.
 *  - namedRange: Named range with the full table (header + data), if any.
 *  - heading   : Section heading in the HTML report (presentation only).
 ...
 *  - Columns = 1-based indices to render as percentage bars.
 */
var BAR_CONFIG = {
  HealthTotal: {
    columns: [2],
...
function onOpen() {
  ...
  var menu = ui.createMenu('📊 HENP Deployment Health Tools')
...
function openWebApp() {
  ...
  var html =
    ...
    '<p>The HENP Deployment Health Web App will open in a new tab.</p>' +
...
/**
 * Master pipeline (snapshot-based) for HENP:
...
function debugShowCellValues() {
  ...
  var sections = sheetNames.map(function (sheetName) {
    ...
    return (
      '<h3 style="margin-top:16px;">Sheet: ' + escapeHtmlDebug_(sheetName) + '</h3>' +
