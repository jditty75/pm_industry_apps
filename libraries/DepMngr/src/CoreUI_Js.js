...
function _CoreUI_Js_getJsBundle() {
  ...
  'var execSummaryPasteHandlerInstalled = false;',
  ...
  '  installExecSummaryPasteHandler();',
  ...
  '',
  '// ============================================================',
  '// EXECUTIVE SUMMARY — PASTE SANITIZER (F1)',
  '// ============================================================',
  '',
  'function installExecSummaryPasteHandler() {',
  '  if (execSummaryPasteHandlerInstalled) return;',
  '  var editor = document.getElementById(\'exec-editor\');',
  '  if (!editor) return;',
  '  editor.addEventListener(\'paste\', handleExecSummaryPaste);',
  '  execSummaryPasteHandlerInstalled = true;',
  '}',
  '',
  'function handleExecSummaryPaste(event) {',
  ...
  '    cleaned = sanitizeExecSummaryPaste(html);',
  '  } else {',
  '    var text = cd.getData(\'text/plain\') || \'\';',
  '    cleaned = plainTextToHtml_(text);',
  '  }',
  '  if (!cleaned) return;',
  ...
  '}',
  '',
  'function sanitizeExecSummaryPaste(html) {',
  '  // Strip MSO conditional comments before parsing',
  '  html = html.replace(/<!--\\[if[\\s\\S]*?<!\\[endif\\]-->/gi, \'\');',
  '  html = html.replace(/<!--[\\s\\S]*?-->/g, \'\');',
  '  // Parse into a detached DOM (does not execute scripts or load resources)',
  '  var doc = document.implementation.createHTMLDocument(\'\');',
  '  doc.body.innerHTML = html;',
  '  // Unwrap Google Docs top-level <b style="font-weight:normal"> wrapper',
  '  var docsWrappers = doc.body.querySelectorAll(\'b[style*="font-weight"]\');',
  '  for (var i = 0; i < docsWrappers.length; i++) {',
  '    var w = docsWrappers[i];',
  '    if (/font-weight\\s*:\\s*normal/i.test(w.getAttribute(\'style\') || \'\')) {',
  '      unwrapElement_(w);',
  '    }',
  '  }',
  '  // Remove disallowed elements outright',
  '  var kill = \'style,script,meta,link,title,head,xml,img,svg,video,iframe,embed,object,\' +',
  '             \'table,thead,tbody,tr,td,th,form,input,button,select,textarea\';',
  '  var toKill = doc.body.querySelectorAll(kill);',
  '  for (var j = toKill.length - 1; j >= 0; j--) {',
  '    toKill[j].parentNode.removeChild(toKill[j]);',
  '  }',
  '  walkAndNormalize_(doc.body);',
  '  rebuildWordLists_(doc.body);',
  '  collapseWhitespace_(doc.body);',
  '  return doc.body.innerHTML;',
  '}',
  '',
  'function unwrapElement_(el) {',
  ...
  '}',
  '',
  'function walkAndNormalize_(root) {',
  '  var UNWRAP_TAGS = { span: 1, div: 1, font: 1, section: 1, article: 1 };',
  '  var ALLOWED_TAGS = { p: 1, br: 1, strong: 1, em: 1, u: 1, ol: 1, ul: 1, li: 1, h2: 1, h3: 1, h4: 1, a: 1 };',
  '  var ownerDoc = root.ownerDocument;',
  '  // querySelectorAll returns document order; reverse for bottom-up processing so children',
  '  // are handled before parents, preventing double-processing of unwrapped descendants.',
  '  var allNodes = Array.prototype.slice.call(root.querySelectorAll(\'*\'));',
  '  for (var i = allNodes.length - 1; i >= 0; i--) {',
  '    var node = allNodes[i];',
  '    if (!node.parentNode) continue;',
  '    var tag = node.tagName.toLowerCase();',
  '    if (tag === \'b\') {',
  '      var repl = ownerDoc.createElement(\'strong\');',
  '      while (node.firstChild) repl.appendChild(node.firstChild);',
  '      node.parentNode.replaceChild(repl, node);',
  '    } else if (tag === \'i\') {',
  '      var repl = ownerDoc.createElement(\'em\');',
  '      while (node.firstChild) repl.appendChild(node.firstChild);',
  '      node.parentNode.replaceChild(repl, node);',
  '    } else if (UNWRAP_TAGS[tag]) {',
  '      unwrapElement_(node);',
  ...
  '}',
  '',
  '// Word bullet character set used for detection and strip (F1.1).',
  '// U+00B7 · MIDDLE DOT is the common Word Symbol-font bullet (the F1.1 regression root cause).',
  '// U+002D - HYPHEN-MINUS and U+002A * ASTERISK are placed first so the resulting character',
  '// class treats them as literals, not as range operators.',
  'var WORD_BULLET_CHARS = \'\\u002D\\u002A\\u00B7\\u2022\\u25E6\\u25AA\\u25CF\\u25CB\\u2013\\u2014\';',
  '// Strips: optional leading whitespace/nbsp, one bullet char, required trailing whitespace/nbsp.',
  '// Trailing gap REQUIRED so hyphen-minus / asterisk are not false-matched mid-sentence.',
  'var WORD_BULLET_STRIP_RE = /^[\\s\\u00A0]*[\\u002D\\u002A\\u00B7\\u2022\\u25E6\\u25AA\\u25CF\\u25CB\\u2013\\u2014][\\s\\u00A0]+/;',
  'var WORD_NUMBER_STRIP_RE = /^[\\s\\u00A0]*(\\d+|[a-z]|[ivxlcdm]+)[.)][\\s\\u00A0]+/i;',
  '',
  '/**',
  ' * Strips the leading bullet or numeric marker from the first text node of a list item.',
  ' * Uses a TreeWalker to find the first non-whitespace-only text node regardless of nesting,',
  ' * removes leading whitespace-only text nodes, then applies the appropriate strip regex.',
  ' * Replaces the flawed F1 stripMarker() which only checked p.firstChild directly.',
  ' *',
  ' * @param {HTMLElement}      li    The <li> element to strip.',
  ' * @param {\'bullet\'|\'number\'} kind  Which regex to apply.',
  ' */',
  'function stripLeadingListMarker_(li, kind) {',
  '  var walker = li.ownerDocument.createTreeWalker(li, NodeFilter.SHOW_TEXT, null);',
  '  var node = walker.nextNode();',
  '  while (node && node.nodeValue.replace(/[\\s\\u00A0]/g, \'\') === \'\') {',
  '    var nextNode = walker.nextNode();',
  '    node.parentNode.removeChild(node);',
  '    node = nextNode;',
  '  }',
  '  if (!node) return;',
  '  var re = (kind === \'number\') ? WORD_NUMBER_STRIP_RE : WORD_BULLET_STRIP_RE;',
  ...
  '}',
  '',
  'function rebuildWordLists_(root) {',
  '  function getListType(p) {',
  '    var text = (p.textContent || \'\').replace(/^[\\u00a0\\s]+/, \'\');',
  '    if (!text) return null;',
  '    var first = text.charAt(0);',
  '    // \'o\' only detected as bullet if followed by 2+ whitespace/nbsp (Word level-2 bullet)',
  '    if (first === \'o\' && /^o[\\u00a0\\s]{2}/.test(text)) return \'ul\';',
  '    if (WORD_BULLET_CHARS.indexOf(first) !== -1) return \'ul\';',
  '    if (WORD_NUMBER_STRIP_RE.test(text)) return \'ol\';',
  '    return null;',
  '  }',
  '',
  ...
  '    if (child.nodeType === 1 && child.tagName.toLowerCase() === \'p\' && getListType(child)) {',
  '      // Collect consecutive list paragraphs',
  '      var run = [];',
  '      var listType = getListType(child);',
  '      while (i < children.length) {',
  '        var c = children[i];',
  '        if (c.nodeType === 1 && c.tagName.toLowerCase() === \'p\' && getListType(c)) {',
  '          run.push(c);',
  '          i++;',
  '        } else {',
  '          break;',
  '        }',
  '      }',
  '      // Build the list and convert each <p> to <li>',
  '      var list = root.ownerDocument.createElement(listType);',
  '      for (var r = 0; r < run.length; r++) {',
  '        var para = run[r];',
  '        var li = root.ownerDocument.createElement(\'li\');',
  '        while (para.firstChild) li.appendChild(para.firstChild);',
  '        // Strip the leading marker AFTER children are in <li> so the TreeWalker',
  '        // can descend into nested inline elements (e.g. <strong>) to find the text node.',
  '        stripLeadingListMarker_(li, listType === \'ol\' ? \'number\' : \'bullet\');',
  '        list.appendChild(li);',
  '        para.parentNode.removeChild(para);',
  '      }',
  '      // Insert the new list where the run started',
  '      var nextSib = children[i] || null;',
  '      if (nextSib && nextSib.parentNode === root) {',
  '        root.insertBefore(list, nextSib);',
  '      } else {',
  ...
  '}',
  '',
  'function collapseWhitespace_(root) {',
  '  function walkText(node) {',
  '    if (node.nodeType === 3) {',
  '      var val = node.nodeValue;',
  '      // Collapse runs of 2+ non-breaking spaces to a single regular space',
  '      val = val.replace(/\\u00a0\\u00a0+/g, \' \');',
  '      // Collapse whitespace runs inside text nodes',
  '      val = val.replace(/[ \\t\\r\\n]+/g, \' \');',
  '      node.nodeValue = val;',
  ...
  '',
  '// ============================================================',
  '// MGM / PGL TAB (feature/mgm-pgl)',
  '// ============================================================',
  '',
  '// State',
  'var mgmPglData   = null;  // { windowDays, today, startDate, endDate, rows, exceptions }',
  'var mgmPglFilter = { type: \'all\', search: \'\' };',
  'var mgmPglWindow = \'next30\';  // \'next30\' | \'thisMonth\' | \'nextMonth\' | \'thisQuarter\' | \'nextQuarter\'',
  '',
  '/**',
  ' * Changes the time window, updates the segmented control, and reloads data.',
  ' * @param {string} windowKey',
  ' */',
  'function setMgmPglWindow(windowKey) {',
  '  mgmPglWindow = windowKey;',
  '  document.querySelectorAll(\'[data-mgmPgl-window]\').forEach(function(btn) {',
  '    var w = btn.getAttribute(\'data-mgmPgl-window\');',
  '    if (w === windowKey) btn.classList.add(\'active\');',
  ...
  '',
  '/**',
  ' * Entry-point: loads MGM/PGL data from the server and renders the tab.',
  ' * Called on tab switch and on window change (always reloads).',
  ' */',
  'function loadMgmPglTab() {',
  '  var tabEl = document.getElementById(\'mgmPgl-tab\');',
  '  if (!tabEl) return;',
  '',
  '  var loadInd = document.getElementById(\'mgmpgl-loading-indicator\');',
  '  var asOf    = document.getElementById(\'mgmpgl-as-of\');',
  '  if (loadInd) loadInd.classList.remove(\'hidden\');',
  '  if (asOf)    asOf.textContent = \'\';',
  '',
  '  // Show skeleton while loading.',
  '  var tbody = document.getElementById(\'mgmpgl-tbody\');',
  '  if (tbody) tbody.innerHTML = \'<tr><td colspan="7" class="empty-state"><div class="report-loading"><div class="spinner-large"></div><p>Loading&hellip;</p></div></td></tr>\';',
  '',
  '  var vmo = getViewModeOpts_() || { viewMode: \'all\', ddDisplayName: \'\' };',
  '  vmo.window = mgmPglWindow;',
  '',
  '  google.script.run',
  '    .withSuccessHandler(function(data) {',
  '      mgmPglData = data || { windowDays: 30, today: \'\', startDate: \'\', endDate: \'\', rows: [], exceptions: [] };',
  '      mgmPglFilter = { type: \'all\', search: \'\' };',
  ...
  '      var _winKeyLabel = _winKeyLabels[mgmPglData.windowKey] || mgmPglData.windowKey || \'\';',
  '      var winLabel = \'Window: \' + _winKeyLabel +',
  '                     \' (from \' + (mgmPglData.startDate || mgmPglData.today || \'\') +',
  '                     \' to \' + (mgmPglData.endDate || \'\') + \')\' +',
  '                     \' \u00B7 loaded \' + _fmtNow_();',
  '      if (asOf) asOf.textContent = winLabel;',
  '    })',
  '    .withFailureHandler(function(err) {',
  '      if (loadInd) loadInd.classList.add(\'hidden\');',
  '      var tbody2 = document.getElementById(\'mgmpgl-tbody\');',
  '      if (tbody2) tbody2.innerHTML = \'<tr><td colspan="7" style="text-align:center;padding:24px;color:#c62828;">Error: \' + escapeHtml(String(err)) + \'</td></tr>\';',
  '      showToast(\'Error loading MGM/PGL data: \' + err, \'error\');',
  '    })',
  '    .getUpcomingSurveysData(vmo);',
  '}',
  '',
  '/**',
  ' * Sets the survey-type chip filter (\'all\' | \'MGM\' | \'PGL\') and re-renders.',
  ' * @param {string} type',
  ' */',
  'function setMgmPglFilterType(type) {',
  '  mgmPglFilter.type = type;',
  '  _syncMgmPglTypeChips_();',
  '  renderMgmPglTable_();',
  '}',
  '',
  '/** Syncs the active class on type chip buttons. @private */',
  'function _syncMgmPglTypeChips_() {',
  '  document.querySelectorAll(\'[data-mgmpgl-type]\').forEach(function(btn) {',
  '    var t = btn.getAttribute(\'data-mgmpgl-type\');',
  '    if (t === mgmPglFilter.type) {',
  '      btn.classList.add(\'active\');',
  '    } else {',
  '      btn.classList.remove(\'active\');',
  '    }',
  ...
  '}',
  '',
  '/** Applies the text search filter and re-renders. */',
  'function searchMgmPgl() {',
  '  var el = document.getElementById(\'mgmpgl-search\');',
  '  mgmPglFilter.search = el ? el.value.trim().toLowerCase() : \'\';',
  '  renderMgmPglTable_();',
  '}',
  '',
  ...
  '',
  '/**',
  ' * Builds the Contacts cell HTML for one MGM/PGL row.',
  ' * Renders one line per contact with mailto links and role labels.',
  ' * @param {Object} row  Survey row from getUpcomingSurveys.',
  ' * @return {string}  Inner HTML for the Contacts <td>.',
  ' * @private',
  ' */',
  'function _buildMgmPglContactsHtml_(row) {',
  '  var lines = [];',
  '',
  ...
  '  }',
  '',
  '  pushList(row.projectManagerContacts);',
  '  pushList(row.execSponsorContacts);',
  '',
  '  if (row.wdSponsor && row.wdSponsor.name) {',
  '    var wd = row.wdSponsor;',
  '    var wdEmail = wd.email || \'\';',
  '    var wdName  = wd.name  || \'\';',
  '    var wdRole  = wd.role  || \'Deployment Sponsor\';',
  '    var wdLink  = wdEmail',
  '      ? \'<a href="mailto:\' + escapeAttr(wdEmail) + \'">\' + escapeHtml(wdName) + \'<\/a>\'',
  '      : escapeHtml(wdName);',
  '    lines.push(wdLink + \' (\' + escapeHtml(wdRole) + \')\');',
  '  }',
  '',
  '  pushList(row.engagementManagers);',
  '',
  ...
  '',
  '/**',
  ' * Renders the MGM/PGL survey rows table, applying current filters.',
  ' * @private',
  ' */',
  'function renderMgmPglTable_() {',
  '  var tbody = document.getElementById(\'mgmpgl-tbody\');',
  '  if (!tbody) return;',
  '',
  '  var rows = (mgmPglData && mgmPglData.rows) ? mgmPglData.rows : [];',
  '',
  '  // Apply type filter.',
  '  if (mgmPglFilter.type && mgmPglFilter.type !== \'all\') {',
  '    rows = rows.filter(function(r) { return r.surveyType === mgmPglFilter.type; });',
  '  }',
  '',
  '  // Apply text search.',
  '  if (mgmPglFilter.search) {',
  '    var q = mgmPglFilter.search;',
  '    rows = rows.filter(function(r) {',
  '      return (r.accountName    || \'\').toLowerCase().indexOf(q) !== -1 ||',
  '             (r.deploymentName || \'\').toLowerCase().indexOf(q) !== -1 ||',
  '             (r.productLabel   || \'\').toLowerCase().indexOf(q) !== -1;',
  '    });',
  '  }',
  '',
  '  if (rows.length === 0) {',
  '    tbody.innerHTML = \'<tr><td colspan="7" class="empty-state">No upcoming surveys match the current filters.</td></tr>\';',
  '    return;',
  '  }',
  '',
  '  tbody.innerHTML = rows.map(function(r) {',
  '    // --- Schedule cell: date + survey-type pill ---',
  '    var surveyType = r.surveyType;  // \'MGM\' or \'PGL\'',
  '    var status     = r.status;      // \'Planned\' | \'Actual\' | null',
  '    var pillClass  = \'survey-pill\';',
  '    var titleText;',
  '    if (surveyType === \'MGM\') {',
  '      pillClass += \' survey-pill-mds-planned\';',
  '      titleText  = \'Mid Deployment Survey (Planned)\';',
  '    } else if (status === \'Actual\') {',
  '      pillClass += \' survey-pill-pgl-actual\';',
  '      titleText  = \'Post Go-Live Survey (Actual)\';',
  '    } else {',
  '      pillClass += \' survey-pill-pgl-planned\';',
  '      titleText  = \'Post Go-Live Survey (Planned)\';',
  '    }',
  '    var scheduleHtml =',
  '      \'<div class="schedule-cell">\' +',
  '        \'<span class="schedule-date">\' + escapeHtml(r.scheduledDate || \'\') + \'<\/span>\' +',
  '        \'<span class="\' + pillClass + \'" title="\' + escapeAttr(titleText || \'\') + \'">\' +',
  '          escapeHtml(surveyType === \'MGM\' ? \'MDS\' : surveyType) +',
  '        \'<\/span>\' +',
  '      \'<\/div>\';',
  '',
  '    var urgency = r.daysUntil <= 7  ? \'color:#c62828;font-weight:600;\' :',
  '                 r.daysUntil <= 14 ? \'color:#b45309;\' : \'\';',
  '',
  '    var contactsHtml = _buildMgmPglContactsHtml_(r);',
  '',
  '    return \'<tr>\' +',
  '      \'<td>\' + scheduleHtml + \'<\/td>\' +',
  '      \'<td style="\' + urgency + \'">\' + r.daysUntil + \'d<\/td>\' +',
  '      \'<td>\' + escapeHtml(r.accountName    || \'\') + \'<\/td>\' +',
  '      \'<td class="deployment-cell" title="\' + escapeAttr(r.productLabel || \'\') + \'">\' + escapeHtml(r.deploymentName || \'\') + \'<\/td>\' +',
  '      \'<td style="font-size:12px;">\' + escapeHtml(r.deploymentStartDate     || \'\') + \'<\/td>\' +',
  '      \'<td style="font-size:12px;">\' + escapeHtml(r.deploymentTargetEndDate || \'\') + \'<\/td>\' +',
  '      \'<td style="font-size:12px;">\' + (contactsHtml || \'<span style="color:var(--color-text-subtle);">&mdash;<\/span>\') + \'<\/td>\' +',
  '    \'<\/tr>\';',
  '  }).join(\'\');',
  '}',
  '',
  '/**',
  ' * Renders the exceptions (missing target dates) table.',
  ' * @private',
  ' */',
  'function renderMgmPglExceptions_() {',
  '  var tbody = document.getElementById(\'mgmpgl-exceptions-tbody\');',
  '  var sub   = document.getElementById(\'mgmpgl-exceptions-sub\');',
  '  if (!tbody) return;',
  '',
  '  var exceptions = (mgmPglData && mgmPglData.exceptions) ? mgmPglData.exceptions : [];',
  '',
  '  if (sub) sub.textContent = exceptions.length + \' deployment\' + (exceptions.length === 1 ? \'\' : \'s\');',
  '',
  ...
  '  }',
  '',
  '  tbody.innerHTML = exceptions.map(function(ex) {',
  ...
  '  if (!tab) return;',
  '  tab.innerHTML = \'<div style="text-align:center; padding:3rem;">\' +',
  '                  \'<div class="spinner-large"></div>\' +',
  '                  \'<p style="margin-top:1rem;">Loading overview...</p></div>\';',
  '  google.script.run',
  '    .withSuccessHandler(function(data) {',
  '      overviewData = data;',
  ...
  '    })',
  '    .withFailureHandler(function(err) {',
  '      tab.innerHTML = \'<div style="padding:2rem; color:var(--color-status-red-fg);">\' +',
  '                      \'Error loading overview: \' + escapeHtml(err.toString()) + \'</div>\';',
  '      loadDeploymentsData();',
  '      loadGoLivesData_both();',
  '    })',
  '    .getOverviewData();',
  ...
  '  if (!tab) return;',
  '  if (!data || data.error) {',
  '    tab.innerHTML = \'<div style="padding:2rem; color:var(--color-status-red-fg);">Overview data unavailable.</div>\';',
  '    return;',
  '  }',
  '',
  '  var dep = data.deployments  || {};',
  '  var gl  = data.goLives      || {};',
  '  var ew  = data.executiveWatch || {};',
  '',
  '  // ── KPI strip ────────────────────────────────────────────────────────',
  '  var kpiHtml =',
  '    \'<div class="overview-section">\' +',
  '    \'  <div class="overview-kpi-strip">\' +',
  '    kpiCard_(\'Total Active\', dep.total  || 0, \'\')      +',
  '    kpiCard_(\'Red\',          dep.red    || 0, \'red\')   +',
  '    kpiCard_(\'Yellow\',       dep.yellow || 0, \'yellow\') +',
  '    kpiCard_(\'Green\',        dep.green  || 0, \'green\') +',
  '    kpiCard_(\'Executive Watch\', ew.count || 0, \'watch\') +',
  '    \'  </div>\' +',
  '    \'</div>\';',
  '',
  '  // ── Go-lives strip ───────────────────────────────────────────────────',
  ...
  '    \'<div class="overview-section">\' +',
  '    \'  <div class="overview-kpi-strip">\' +',
  '    kpiCard_(\'Go-Lives (Last 30d)\',   gl.recentCount   || 0, \'\') +',
  '    kpiCard_(\'Go-Lives (Next 30d)\',   gl.upcomingCount || 0, \'\') +',
  '    \'  </div>\' +',
  '    \'</div>\';',
  '',
  '  // ── Top Red list ─────────────────────────────────────────────────────',
  '  var topRedRows = (dep.topRed || []);',
  '  var topRedHtml = \'<div class="overview-list-section">\' +',
  '    \'<div class="overview-list-title">Top Red Deployments</div>\';',
  '  if (topRedRows.length === 0) {',
  '    topRedHtml += \'<div class="overview-empty">No red deployments</div>\';',
  '  } else {',
  '    topRedRows.forEach(function(r) {',
  '      topRedHtml +=',
  '        \'<div class="overview-list-row">\' +',
  '        \'  <span class="status-badge status-red">Red</span>\' +',
  '        \'  <div class="overview-list-main">\' +',
  '        \'    <strong>\' + escapeHtml(r.accountName    || \'\') + \'</strong>\' +',
  '        \'    <span>\' + escapeHtml(r.deploymentName || \'\') + \'</span>\' +',
  '        \'  </div>\' +',
  '        \'  <div class="overview-list-meta">\' +',
  '        \'    <span>\' + escapeHtml(r.partner  || \'\') + \'</span>\' +',
  '        \'    <span>\' + (r.mtpDate ? formatDate(r.mtpDate) : \'—\') + \'</span>\' +',
  '        \'  </div>\' +',
  '        \'</div>\';',
  '    });',
  '  }',
  '  topRedHtml += \'</div>\';',
  '',
  '  // ── Upcoming go-lives list ───────────────────────────────────────────',
  '  var upcomingRows = (gl.upcomingList || []);',
  '  var upcomingHtml = \'<div class="overview-list-section">\' +',
  '    \'<div class="overview-list-title">Upcoming Go-Lives (Next 30d)</div>\';',
  '  if (upcomingRows.length === 0) {',
  '    upcomingHtml += \'<div class="overview-empty">No upcoming go-lives</div>\';',
  '  } else {',
  ...
  '',
  '  // ── Stage strip ──────────────────────────────────────────────────────',
  '  var stages    = dep.byStage || [];',
  '  var stageHtml = \'<div class="overview-section">\' +',
  '    \'<div class="overview-section-title">Active Deployments by Stage</div>\' +',
  '    \'<div class="overview-stage-strip">\';',
  '  if (stages.length === 0) {',
  '    stageHtml += \'<span class="overview-empty">No stage data</span>\';',
  '  } else {',
  '    stages.forEach(function(s) {',
  '      stageHtml +=',
  '        \'<div class="overview-stage-item">\' +',
  '        \'  <div class="overview-stage-count">\' + (s.count || 0) + \'</div>\' +',
  '        \'  <div class="overview-stage-label">\' + escapeHtml(s.stage || \'\') + \'</div>\' +',
  '        \'</div>\';',
  '    });',
  '  }',
  '  stageHtml += \'</div></div>\';',
  ...
  '    kpiHtml +',
  '    glHtml  +',
  '    \'<div class="overview-lists">\' + topRedHtml + upcomingHtml + \'</div>\' +',
  '    stageHtml;',
  '}',
  '',
  'function kpiCard_(label, value, modifier) {',
  '  var cls = \'overview-kpi-card\' + (modifier ? \' overview-kpi-\' + modifier : \'\');',
