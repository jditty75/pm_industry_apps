...
function _CoreUI_Js_getJsBundle() {
  ...
  'var useDdFromContacts = !!UI_DT.useDdFromContacts;  // D1: SLG-only flag',
  ...
  '',
  '/**',
  ' * D1: Resolve the Delivery Director display string for a deployment row.',
  ' * Priority: (1) existing override in row.deliveryDirector, (2) SFDC_Contacts',
  ' * derived value in row.ddFromContacts, (3) empty string.',
  ' * Only called when useDdFromContacts is true (SLG-only in V1).',
  ' *',
  ' * @param {Object} row  Deployment row from APP_DATA.deployments.',
  ' * @return {string}',
  ' */',
  'function resolveDeploymentOwner_(row) {',
  '  if (row.deliveryDirector && row.deliveryDirector.toString().trim()) {',
  '    return row.deliveryDirector.toString().trim();',
  '  }',
  '  if (row.ddFromContacts && row.ddFromContacts.toString().trim()) {',
  '    return row.ddFromContacts.toString().trim();',
  '  }',
  '  return \'\';',
  '}',
  '',
  ...
  '    var deliveryDirector = useDdFromContacts',
  '      ? resolveDeploymentOwner_(row)',
  '      : (row.deliveryDirector && row.deliveryDirector.toString().trim() !== \'\'',
