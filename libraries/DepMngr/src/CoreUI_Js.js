...
function _CoreUI_Js_getJsBundle() {
  ...
  ' * Priority (SLG only, when useDdFromContacts=true):',
  ' *   (1) row.deliveryDirector override, (2) row.ddFromContacts from SFDC_Contacts.',
  ' * HENP/HC: useDdFromContacts is false — falls straight through to row.deliveryDirector.',
  ' *',
  ' * Inlined at call site rather than called as a function so the branch is',
  ' * evaluated once per row with no extra function-call overhead.',
  ' */',
  '',
  ...
  '    var ownerText;',
  '    if (useDdFromContacts) {',
  '      if (row.deliveryDirector && row.deliveryDirector.trim()) {',
  '        ownerText = row.deliveryDirector.trim();',
  '      } else if (row.ddFromContacts && row.ddFromContacts.trim()) {',
  '        ownerText = row.ddFromContacts.trim();',
  '      } else {',
  '        ownerText = \'\';',
  '      }',
  '    } else {',
  '      ownerText = row.deliveryDirector || \'\';',
  '    }',
  '    var deliveryDirector = ownerText;',
